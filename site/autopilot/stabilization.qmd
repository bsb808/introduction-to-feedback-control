...
1. **Stick → setpoint scaling.** The controller's input is a target value (also known as a *setpoint*, *command*, *reference*, or *desired value*). The pilot provides this target through the R/C sticks: each stick's PWM signal is normalized to a *stick input* in the range [−1, +1], then multiplied by a scaling parameter (`CRUISE_SPEED` for the throttle / speed channel, `ACRO_TURN_RATE` for the steering / yaw-rate channel) to produce the channel-specific setpoint in physical units.

   Set these scaling values slightly above the maximum response you actually want from the boat so the full stick range maps to the full *useful* range of the plant. For our baseline setup `CRUISE_SPEED = 5` m/s and `ACRO_TURN_RATE = 360` deg/s — the latter is intentionally too large to expose this point; the procedure has you reduce it to a value the boat can physically achieve.

   `ACRO_TURN_RATE` is specified in **deg/s** as a parameter, but ArduRover converts it to **rad/s** before passing it into the yaw-rate loop. So the parameter file shows `ACRO_TURN_RATE = 360`, while the `STER_DesTurnRate` channel in the log is in rad/s (full-stick → `360 × π/180 ≈ 6.28` rad/s).

2. **Rate limiter on the commanded setpoint** (`ATC_ACCEL_MAX` / `ATC_STR_ACC_MAX`). Caps how fast the setpoint can change, ramping a stick step at the maximum rate instead of letting it jump instantaneously. Baseline values: `ATC_ACCEL_MAX = 10` m/s² (speed), `ATC_DECEL_MAX = 0` (falls back to ACCEL_MAX), `ATC_STR_ACC_MAX = 0` deg/s² (yaw — already disabled). For step-response analysis we want to see the loop's true step response, so the lab also disables the speed-channel limiter (`ATC_ACCEL_MAX = 0`).
...
4. **Error junction** subtracts the measured value from the (filtered) setpoint to form the error signal that drives the P, I, and D terms.
...
6. **PID controller** with separate P, I, D, and FF terms. The gains start from ArduRover's install defaults, except where the lab-baseline `.param` deliberately deviates. Baseline values: `ATC_SPEED_P = 0.2`, `ATC_SPEED_I = 0.2`, `ATC_SPEED_D = 0`, `ATC_SPEED_FF = 0`; `ATC_STR_RAT_P = 0.2`, `ATC_STR_RAT_I = 0.2`, `ATC_STR_RAT_D = 0`, `ATC_STR_RAT_FF = 0`. The **Base** experiment in Lab 2 leaves all of these alone, so with FF zeroed on both channels there is no feedforward contribution. The **FeedForward** experiment restores the FF gains; **Tuned** also adjusts P / I / D.

7. **Output sum** combines the PID output with any baseline FF and sends it to the actuator. The speed loop adds a `CRUISE_THROTTLE` baseline here (which we leave at `0` for the lab); the yaw loop has no equivalent baseline.
...
![Generic PID control loop with feedforward and setpoint shaping. Numbers ① – ⑦ correspond to the 7 building blocks listed above.](images/generic_pid_loop.svg){fig-alt="Generic PID block diagram with setpoint scaling, acceleration limiter, target LPF, error junction, error LPF, PID controller, output sum, plant, and feedback loop, with a feedforward branch from the setpoint to the output sum."}
