...
For the PID-tuning labs we use the DataFlash log: it has higher rate, more channels, and full PID-internal state via `PIDS_*` (speed) and `PIDA_*` (yaw-rate / attitude).
...
| `PIDS_Tar` | PID's internal target (after rate limiting, when active) | m/s |
| `PIDS_Act` | PID's input measurement | m/s |
| `PIDS_Err` | Target − Act, after error filter | m/s |
| `PIDS_P`, `_I`, `_D`, `_FF` | Individual PID term outputs | normalized |
| `PIDS_DFF`, `_Dmod`, `_SRate`, `_Flags` | Auxiliary fields (advanced features and limiters) | — |

ArduPilot does **not** log a separate `Out` field — the total controller output is `P + I + D + FF` (optionally `+ DFF`). `THR_DesSpeed` and `PIDS_Tar` differ only when an upstream filter or rate limiter is active. With the lab settings (acceleration limiters disabled, FLTT off), the two are essentially identical.
...
| `PIDA_Tar` | PID's internal target (after rate limiting, when active) | rad/s |
| `PIDA_Act` | PID's input measurement | rad/s |
| `PIDA_Err` | Target − Act, after error filter | rad/s |
| `PIDA_P`, `_I`, `_D`, `_FF` | Individual PID term outputs | normalized |
| `PIDA_DFF`, `_Dmod`, `_SRate`, `_Flags` | Auxiliary fields | — |
