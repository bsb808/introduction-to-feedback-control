...
\begin{itemize}
  ...
  \item A faster rise time can be ``achieved'' simply by increasing $R_{\max}$---without any change to the PID gains.  This conflates actuator smoothing with controller performance.
\end{itemize}
...
% CLAUDE: Revise this process.   We are going to try connecting the ARM switch so that it restarts a log file, so it will be important for the scribe to write down the times of the Arming and what experiments were done during that log.  Also, we won't go into ACRO mode until after we launch the boat.  Finally, it is good to both manually write down the various PID parameters and save a timestamped (or labelled) export of the params file from mission planner.
\begin{enumerate}
...
\begin{enumerate}
  \item Surge step trials (2--3 successful runs):
    \begin{itemize}
      \item Put in ACRO model, note time starting new log file.
...
% CLAUDE: Go into some detail on an ad hoc tuning process.  Refer to the end of the PID article with hints for tuning.  Also mention how to measure "performance" in the context of minimizing time between waypoints.

\subsection{Run 2: Tuned Parameters}

After you have tuned the gains for the speed or heading loop, capture the results with a clean run.


\begin{enumerate}
  \item As a team, select new values for the six tunable gains. 
    Use your Lab~1 plant models to simulate the predicted closed-loop response for each channel before choosing values.  Record your chosen parameters and the reasoning in your field notes before entering the water.
