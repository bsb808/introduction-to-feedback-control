...
\begin{enumerate}[label=(\alph*)]
  ...
  \item What kind of engineering system has dynamics like this --- one real
    pole, two complex poles, no zeros?  Either give a one-sentence example,
    or argue that $G(s)$ here is most naturally read as a \emph{plant --
    controller cascade}, $G(s) = C(s)\,P(s)$.  (Recall the chapter's note on
    overloaded notation.) % CLAUDE: No.  I meant to give a physical interpretation (one sentence) in the problem definition.
\end{enumerate}
...
For each computational part below, work the answer to each of eleement first \emph{analytically,
by hand,}, then repeat and verify in MATLAB.
...
\begin{enumerate}[label=(\alph*)]
  ...
  \item Find the steady-state error for the inputs $5u(t)$, $5t\,u(t)$, and
    $5t^2 u(t)$, by hand.  Verify the each response with MATLAB. 
