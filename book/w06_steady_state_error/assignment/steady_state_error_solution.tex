...
\[
  G(s) = C(s)\,G_p(s) = \frac{K_p\,(s+5)}{s\,(s+8)(s+12)},
\]
...
\]
As $s \to 0$, the open-loop transfer function is dominated by its
integrator,
\[
...
\textbf{(c)}  \texttt{pole(feedback(Kp*Gp, 1))} with $K_p = 480$ returns
roots near $-4.46 \pm 14.2j$ and $-11.1$ --- all in the left half-plane,
so the closed loop is stable.  This is what makes the SSE result in (b)
meaningful: the chapter's formula assumes a stable closed loop.
